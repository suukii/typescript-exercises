        ...
        requestUsers(usersResponse => {
            ...
            requestCurrentServerTime(serverTimeResponse => {
                ...
                requestCoffeeMachineQueueLength(
                    coffeeMachineQueueLengthResponse => {
                        console.log(
                            chalk.yellow('Coffee machine queue length:'),
                        )
                        if (
                            coffeeMachineQueueLengthResponse.status ===
                            'success'
                        ) {
                            console.log(
                                `   ${coffeeMachineQueueLengthResponse.data}`,
                            )
                        } else {
                            return callback(
                                new Error(
                                    coffeeMachineQueueLengthResponse.error,
                                ),
                            )
                        }
