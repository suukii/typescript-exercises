...
async function testUsersDatabase() {
    const usersDatabase = new Database<User>(
    ...
    expect(
        (
            await usersDatabase.find({ occupation: { $eq: 'Magical entity' } })
        ).map(({ _id }) => _id),
    ...
    expect(
        (await usersDatabase.find({ age: { $in: [31, 99] } })).map(
            ({ _id }) => _id,
        ),
